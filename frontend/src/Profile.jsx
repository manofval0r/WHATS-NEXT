...
export default function Profile() {
  ...
  const handleSaveSocials = async () => {
    try {
      const token = localStorage.getItem('access_token');
      await axios.post('http://127.0.0.1:8000/api/profile/update-socials/', socialForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setEditingSocials(false);
      fetchProfile();
    } catch (e) { alert("Failed to save socials"); }
  };

  const handleSaveCVText = async (itemId) => {
    try {
      const token = localStorage.getItem('access_token');
      await axios.post(`http://127.0.0.1:8000/api/profile/update-cv/${itemId}/`, { text: editText }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setEditingItem(null);
      fetchProfile();
    } catch (e) { alert("Failed to save text"); }
  };
