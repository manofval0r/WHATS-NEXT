...
export default function Community() {
    ...
    const toggleComments = async (projectId) => {
        ...
        if (!comments[projectId]) {
            ...
            try {
                const token = localStorage.getItem('access_token');
                const res = await axios.get(`http://127.0.0.1:8000/api/community/comments/${projectId}/`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const postComment = async (projectId) => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            const res = await axios.post(`http://127.0.0.1:8000/api/community/comments/${projectId}/create/`,
                { text: newComment },
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const deleteComment = async (commentId, projectId) => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            await axios.delete(`http://127.0.0.1:8000/api/community/comments/${commentId}/delete/`, {
                headers: { Authorization: `Bearer ${token}` }
            });
